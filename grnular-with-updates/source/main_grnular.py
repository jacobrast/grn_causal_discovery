...
sys.path.insert(0,'/home/ubuntu/grnular/utils')
sys.path.insert(0,'/home/ubuntu/grnular/source')
...
from grnular_model import glad_model, get_triu_vec, dnn_model
...
parser.add_argument('--D', type=int, default=400, #1000,
...
def format_torch(data, is_DS1=False):# convert the data to pytorch
    data_torch = []
    typeS = 'mean'
...
def glad_train_batch(train_data, valid_data=None):
    ...
    model_glad = glad_model(args.L, args.theta_init_offset, args.nF, args.H, args.D, len(TF),\
                            USE_CUDA=USE_CUDA) 
    ...
    for i, ri in enumerate(rd):
        ...
        for epoch in range(args.EPOCHS):# in each epoch, go through the complete batch
            ...
            theta_s, theta_true_vec, vae_loss, vae_kl, vae_latent, Z_list = grnular(X, theta_true, TF, model_glad, args) 

            unrolling_loss = acc_loss(Z_list, theta_true_vec, dn=0.9)
            loss = unrolling_loss + vae_loss
            theta_s = torch.squeeze(theta_s)

            if epoch % args.PRINT_EPOCH == 0:
                print('\n Epoch = ', epoch, 'Batch = ',batch_num, ' loss glad = ', loss.detach().cpu().numpy())
...
def glad_predict_batch(model, data, PRINT=True, PREDICT_TF=False, PICKLE = False, train_time = None):
    ...
    with torch.no_grad():
        ...
        if PICKLE:
            FILEPATH = "DS2_results_new.pickle" 
...
def acc_loss(Z_list, theta_true, dn=0.9, dtype = torch.cuda.FloatTensor):
    num_unrolls, _, = Z_list.size()
    
    # cumulative loss
    factor = torch.tensor([dn ** i for i in range(num_unrolls, 0, -1)]).type(dtype)
    loss = torch.sum(factor * torch.sum((Z_list - theta_true) ** 2, dim=1) / torch.sum(theta_true ** 2))
...
def glad_predict_single(model, model_auxiliary, theta_true, X, TF, PRINT=False, pair_num=-1, BEELINE=False):
    ...
    theta_s, theta_true_vec, vae_loss, vae_kl, vae_latent, Z_list = grnular(X, theta_true, TF, model_glad, args, PREDICT=True) 
    theta_s = torch.squeeze(theta_s)
    ...
    itr_details = [loss.detach().cpu().numpy()]
    if PRINT:
...
# %%
def load_saved_data(args, is_DS1 = False):
    FILEPATH = '/home/ubuntu/saved_data/KTr20_KVa20_KTe50_D400_C9_Sp0.1_Dtclean_ppc300_SS15_NP0.1_De5_NTdpd_SCS2_pcrln0.2_pcrlx0.5_pcrhn0.7_pcrhx1.0_kmin1.0_kmax5.0_rMR0.1_TFp0.2_DS2.pickle'
