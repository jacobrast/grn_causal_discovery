...
from grnular_model import dnn_model
...
def get_prod_weights(model_dnn):
    ...
    generator_of_params = list(model_dnn.DNN.named_parameters())
    passing = [T[1] for T in generator_of_params if 'weight' in T[0]]
    passing.insert(0, torch.eye(passing[0].shape[1]).to(device = 'cuda'))
    
    multiply = lambda acc, item: torch.matmul(acc, torch.abs(item).t())

    res = list(accumulate(passing, multiply))[-1]
    return res 
...
def reconstruct_adj(Z, TF, dtype):# Z = TxD
    T, D = Z.shape
    # reconstruct the adj matrix
    theta_s = torch.zeros(D, D).type(dtype)
    theta_st = torch.zeros(D, D).type(dtype)
    theta_s[TF, :D] = Z
    theta_st[:D, TF] = Z.t()
...
def grnular(X, theta_true, TF, models, args, PRINT=True, PREDICT=False):
    ...
    theta_true_full = theta_true
    theta_true = get_OT_submatrix(theta_true, TF)
    theta_true_vec = torch.flatten(theta_true)

    Z_list = torch.empty(size=(args.L, theta_true_vec.shape[0])).type(dtype)

    for k in range(args.L - 1):

        theta_k, model_details = optDNN(model_details, X, Z.detach(), TF, lambda_k.detach(), args,\
                                        do_PRINT, PREDICT)# DxT

        Z = model_glad.eta_forward(theta_k, St, k, Z)#.detach() 

        # update the lambda
        F_norm = torch.Tensor([get_frobenius_norm(Z - theta_k, single=True)]).type(dtype)
        lambda_k = model_glad.lambda_forward(F_norm, lambda_k, k)

        #loss_glad += criterion_graph(Z, theta_true, PRINT=do_PRINT)/args.L
        Z_list[k, :] = torch.flatten(Z)

    k += 1
    theta_k, model_details = optDNN(model_details, X, Z.detach(), TF, lambda_k.detach(), args, True, PREDICT)# DxT

    Z, vae_loss, vae_kl, vae_latent = model_glad.vae.forward(Z, theta_true) 
    Z_list[k, :] = torch.flatten(Z)

    return reconstruct_adj(Z, TF, dtype), theta_true_vec, vae_loss, vae_kl, vae_latent, Z_list
